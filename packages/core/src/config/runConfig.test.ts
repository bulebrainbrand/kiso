...
describe("runConfig", () => {
  it("default exportのオブジェクトを読み込める", async () => {
    ...
    expect(await runConfig(path)()).toBeRight({ default: { foo: "bar" } });
  });
  ...
  it("TypeScript構文を含むconfigを読み込める", async () => {
    ...
    expect(await runConfig(path)()).toBeRight({ default: { x: 1 } });
  });

  it("プリミティブのdefault exportを読み込める", async () => {
    const path = writeConfig(makeTempRoot(), "export default 42;\n");
    expect(await runConfig(path)()).toBeRight({ default: 42 });
  });
