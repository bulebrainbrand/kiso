...
const toTestFile = (file: string): string =>
  path.join(
    path.dirname(file),
    path.basename(file).slice(0, -path.extname(file).length) + ".test" + path.extname(file),
  );
const targetFiles = fullStr
  .split("\n")
  .filter((str) => str.length !== 0)
  .filter((str) => ignorePatterns.every((pattern) => !matchesGlob(str, pattern)))
...
for (const [i, file] of testFiles.entries()) {
  ...
  const coverage = JSON.parse(readFileSync("./coverage/coverage-summary.json").toString());
